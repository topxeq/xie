...
var InstrNameSet map[string]int = map[string]int{
	...
	"strJoinLines": 1537, // 连接一个字符串数组，中间以指换行符（\n）分隔
	...
	"getWebHead": 20215, // 仅获取URL响应头，这样可以验证网络资源是否存在而不必实际下载

	"urlExists": 20217, // 判断一个网络资源是否存在，用法示例：urlExists $rs URL，有可能返回error对象
	...
	"genFileList": 21901, // 生成目录中的文件列表，即获取指定目录下的符合条件的所有文件，例：getFileList $result `d:\tmp` "-recursive" "-pattern=*" "-exclusive=*.txt" "-withDir" "-verbose"，另有 -compact 参数将只给出Abs、Size、IsDir三项, -limit=3将只列出指定的（这里是3）个文件, -dirOnly参数将只列出目录（不包含文件），列表项对象内容类似：map[Abs:D:\tmpx\test1.gox Ext:.gox IsDir:false Mode:-rw-rw-rw- Name:test1.gox Path:test1.gox Size:353339 Time:20210928091734]
	"getFileList": 21901,
...
func RunInstr(p *XieVM, r *RunningContext, instrA *Instr) (resultR interface{}) {
	...
	case 1403: // method/mt
		...
		case time.Time:
			...
			case "getInfo":
				...
				p.SetVar(r, pr, map[string]interface{}{"Time": nv, "Formal": nv.Format(tk.TimeFormat), "Compact": nv.Format(tk.TimeFormatCompact), "Full": fmt.Sprintf("%v", nv), "Year": nv.Year(), "Month": nv.Month(), "Day": nv.Day(), "Hour": nv.Hour(), "Minute": nv.Minute(), "Second": nv.Second(), "Zone": zoneT, "Offset": offsetT, "UnixNano": nv.UnixNano(), "WeekDay": int(nv.Weekday()), "NanoSec": nv.Nanosecond(), "MilliSec": nv.Nanosecond() / 1000000})
				return ""
			case "format":
				var v2 string = ""
	...
	case 1532: // strSplitLines
		...
		s1 := tk.ToStr(p.GetVarValue(r, instrT.Params[v1p]))

		listT := tk.SplitLines(s1)
	...
	case 1537: // strJoinLines
		...
		if ok {
			l3 := tk.AnyArrayToStringArray(l2)
			p.SetVar(r, pr, tk.JoinLines(l3))
			return ""
		}
	...
	case 20215: // getWebHeader
		...
		rs := tk.GetWebHead(tk.ToStr(v2), listT...)

		p.SetVar(r, pr, rs)

		return ""

	case 20217: // urlExists
		if instrT.ParamLen < 2 {
			return p.Errf(r, "not enough parameters(参数不够)")
		}
		...
		rs := tk.UrlExists(tk.ToStr(v2), listT...)
